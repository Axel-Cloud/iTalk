...
const SearchConversations = async (Data, ConversationResponse) => {
    await UserSchema.findById(Data.query.ID, async (Error, User) =>{
        if(!Error){
            let FinalConversations = [];

            await ConversationSchema.find({_id: {"$in":[...User.Conversations]}}).sort([['Conversation.Date', -1]]).exec(async (Error, Conversations) => {
                if(!Error){
